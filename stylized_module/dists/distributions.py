...
class StackedDistribution(TorchDistribution):
    def __init__(
        ...
        *base_dists,
        ...
        self.batch_shape = broadcast_shape(*[base_dist.batch_shape for base_dist in base_dists])
        self.event_shape = base_dists[0].event_shape + (len(base_dists),)
        self.base_dists = tuple(base_dist.expand(batch_shape) for base_dist in base_dists)
        super().__init__(self.batch_shape, self.event_shape, validate_args)
...
def build_priors(bounds: dict[str, float] = params.IM_PARAMETER_DICT):
    priors = []
    for k, v in bounds:
        if k == 'U':
