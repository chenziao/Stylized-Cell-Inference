TITLE Fluctuating conductances NMDA lognormal
...
NEURON {
	POINT_PROCESS Gfluct2NMDAlogn
	RANGE g_e, g_i, E_e, E_i, g_e0, g_i0, g_e1, g_i1
	RANGE std_e, std_i, tau_e, tau_i, D_e, D_i
	RANGE g_n, g_n0, g_n1, std_n, tau_n, D_n
	RANGE g_e0_lg, g_i0_lg, g_n0_lg, std_e_lg, std_i_lg, std_n_lg
	RANGE new_seed
...
ASSIGNED {
	...
	g_n	(umho)
	g_e0_lg         : average excitatory conductance log scale
	g_i0_lg         : average inhibitory conductance log scale
	g_n0_lg
	std_e_lg        : standard dev of excitatory conductance log scale
	std_i_lg        : standard dev of inhibitory conductance log scale
	std_n_lg
...
INITIAL {
	...
	g_n1 = 0
	if (g_e0!=0) {
		g_e0_lg = logn_mu(g_e0, std_e)
		std_e_lg = logn_sigma(g_e0, std_e)
	} else {
		g_e = 0
	}
	if (g_i0!=0) {
		g_i0_lg = logn_mu(g_i0, std_i)
		std_i_lg = logn_sigma(g_i0, std_i)
	} else {
		g_i = 0
	}
	if (g_n0!=0) {
		g_n0_lg = logn_mu(g_n0, std_n)
		std_n_lg = logn_sigma(g_n0, std_n)
	} else {
		g_n = 0
	}
	if(tau_e != 0) {
		D_e = 2 * std_e_lg * std_e_lg / tau_e
		exp_e = exp(-dt/tau_e)
		amp_e = std_e_lg * sqrt( (1-exp(-2*dt/tau_e)) )
	}
	if(tau_i != 0) {
		D_i = 2 * std_i_lg * std_i_lg / tau_i
		exp_i = exp(-dt/tau_i)
		amp_i = std_i_lg * sqrt( (1-exp(-2*dt/tau_i)) )
	}
	if(tau_n != 0) {
		D_n = 2 * std_n_lg * std_n_lg / tau_n
		exp_n = exp(-dt/tau_n)
		amp_n = std_n_lg * sqrt( (1-exp(-2*dt/tau_n)) )
	}
}
...
PROCEDURE oup() {		: use Scop function normrand(mean, std_dev)
	...
	if (g_e0!=0){
		if(tau_e!=0) {
			...
			g_e1 = std_e_lg * rand_e
		}
		g_e = exp(g_e0_lg + g_e1)
	}
	if (g_n0!=0){
		if(tau_n!=0) {
			g_n1 =  exp_n * g_n1 + amp_n * rand_e
		} else {
			g_n1 = std_n_lg * rand_e
		}
		g_n = exp(g_n0_lg + g_n1)
	}
	if (g_i0!=0){
		if(tau_i!=0) {
			g_i1 =  exp_i * g_i1 + amp_i * normrand(0,1)
		} else {
			g_i1 = std_i_lg * normrand(0,1)
		}
		g_i = exp(g_i0_lg + g_i1)
	}
}

FUNCTION logn_mu(mean, std) {
	LOCAL mean2
	: UNITSOFF
	mean2 = mean * mean
	logn_mu = log(mean2 / sqrt(mean2 + std * std))
    : UNITSON
}

FUNCTION logn_sigma(mean, std) {
	: UNITSOFF
	logn_sigma = sqrt(log(1 + (std * std) / (mean * mean)))
	: UNITSON
}
